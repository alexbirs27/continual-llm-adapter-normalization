...
Extending the orthogonal constraint to all six projection modules — q_proj, k_proj, v_proj, o_proj, up_proj, and down_proj — shows that the A matrices maintain a similar degree of inter-task orthogonality regardless of which module is targeted. The per-module analysis shows near-zero cosine similarity and Gram Frobenius scores across all six module types and all 28 transformer layers, suggesting the constraint is equally effective whether applied to attention or MLP projections.
